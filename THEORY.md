...
- **Vertical confidence**: residual-based maps when checkpoints carry Z (ΔZ between checkpoints and surface).
...
4. **Vertical confidence (ΔZ)**: When checkpoints have Z, interpolate surface Z at checkpoint XY (triangulation), compute residuals `delta_z = z_cp - z_surface`, also `|delta_z|`, then IDW these residuals over the grid. Outputs map both signed and absolute residuals.

## Outputs (per surface)
- Horizontal confidence: CSV (distances, weights, combined), heatmap PNG, ranking plot PNG, distance histogram, interactive HTML, footprint PNG.
- Vertical confidence (if Z available): CSV (delta_z, abs_delta_z), heatmaps (delta_z, |delta_z|), histogram, interactive HTML, footprint PNG (shared).
